...
export function CircleCard({ circle, isMember = false, onJoinLeave }: CircleCardProps) {
  ...
  return (
    <div 
      className="bg-slate-50 dark:bg-zinc-950 border border-white/5 rounded-3xl overflow-hidden hover:border-blue-500/30 hover:shadow-[0_0_30px_rgba(59,130,246,0.1)] transition-all cursor-pointer group hover:-translate-y-1"
    ...
    >
      ...
      <div className="h-32 bg-gradient-to-tr from-blue-500/20 to-fuchsia-500/20 relative flex items-center justify-center border-b border-white/5">
        ...
        <div className="w-16 h-16 bg-white dark:bg-zinc-900 border-2 border-slate-200 dark:border-zinc-800 rounded-2xl flex items-center justify-center text-3xl shadow-xl transform group-hover:scale-110 group-hover:rotate-3 transition-transform">
          {circle.iconEmoji}
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-1.5 text-xs font-bold text-slate-500 dark:text-zinc-500 mb-4">
          <MapPin className="w-3.5 h-3.5" />
          {circle.ward}, {circle.city}
        </div>

        <div className="flex items-center gap-4 text-sm font-bold text-slate-500 dark:text-zinc-400 mb-6">
        ...
        {onJoinLeave && (
          <button 
            className={`w-full py-2.5 rounded-xl text-sm font-bold transition-all ${
              isMember 
                ? "bg-white dark:bg-zinc-900 text-slate-500 dark:text-zinc-400 hover:bg-red-500/10 hover:text-red-500" 
