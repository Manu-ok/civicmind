      ...
      <AnimatePresence>
        {showDesktopPicker && (
          <motion.div 
            ...
            className={`hidden md:flex absolute z-30 bottom-full ${compact ? 'left-0' : '-left-2'} mb-2 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-1.5 items-center gap-1`}
              ...
              return (
                <div key={reaction.label} className="relative flex flex-col items-center">
                  <AnimatePresence>
                    {hoveredReaction === reaction.label && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 5 }}
                        className="absolute bottom-full mb-2 bg-black/90 text-white text-[10px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap z-40 border border-zinc-800"
                      >
                        {reaction.meaning}
                      </motion.div>
                    )}
                  ...
                  <motion.button 
                    ...
                    className={`w-11 h-11 text-2xl flex items-center justify-center rounded-xl hover:bg-zinc-800 origin-bottom transition-colors duration-200 ease-in-out ${
                      isSelected ? `bg-zinc-800 ring-2 ${reaction.ring}` : ""
                    }`}
                  >
                    {reaction.emoji}
                  </motion.button>
      ...
      <div className="md:hidden">
        <BottomSheet isOpen={showMobilePicker} onClose={() => setShowMobilePicker(false)}>
          <div className="px-2 pt-2 pb-6">
            <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-6 text-center">Select Reaction</h3>
                ...
                return (
                  <motion.button 
                    ...
                    className={`flex flex-col items-center gap-2 w-full p-4 rounded-2xl transition-colors duration-200 ease-in-out ${
                      isSelected ? `bg-zinc-800 ring-2 ${reaction.ring}` : "hover:bg-zinc-900"
                    }`}
                  >
                    <span className="text-4xl">{reaction.emoji}</span>
                    <span className={`text-[10px] font-semibold uppercase tracking-wider ${isSelected ? reaction.color : 'text-zinc-500'}`}>
                      {reaction.label}
                    </span>
                  </motion.button>
                );
      ...
      >
        {activeReactions.length === 0 ? (
          <div className="flex items-center gap-1 opacity-40 group-hover:opacity-100 transition-opacity px-2">
            ...
            <span className="text-xs font-semibold text-zinc-500 group-hover:text-zinc-300">React</span>
          </div>
        ) : (
          <div className="flex items-center gap-1.5 bg-zinc-900 border border-zinc-800 rounded-full px-4 py-2 group-hover:bg-zinc-800 transition-colors duration-200 ease-in-out">
              ...
              {activeReactions.slice(0, 3).map(([key, _], i) => {
                ...
                return r && (
                  <motion.div 
                    ...
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="relative z-10 w-6 h-6 flex items-center justify-center bg-zinc-800 rounded-full border-2 border-zinc-900 text-sm"
                    style={{ zIndex: 10 - i }}
                  >
                    {r.emoji}
                  </motion.div>
            ...
            <FlipCounter 
              value={totalCount} 
              className={`text-xs font-semibold ${currentReaction ? REACTIONS.find(r => r.label === currentReaction)?.color : "text-zinc-400"}`}
            />
          </div>
        )}
      </div>
