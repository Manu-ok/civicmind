...
export function FeedItemCard({ item }: { item: FeedItem }) {
  ...
  if (item.type === "followed_you") {
    return (
      <div className="bg-slate-50 dark:bg-zinc-950 border border-white/5 rounded-2xl p-4 mb-4 flex items-center gap-4 cursor-pointer hover:bg-white dark:bg-zinc-900/80 transition-colors group" onClick={() => router.push(`/profile/${item.actorUsername}`)}>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <div className="relative w-6 h-6 rounded-full overflow-hidden shrink-0">
              {item.actorPhotoURL ? (
                <Image fill src={item.actorPhotoURL} alt="" className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center text-[10px] font-bold text-slate-500 dark:text-zinc-500">
            ...
            <span className="text-slate-500 dark:text-zinc-500 text-sm">started following you</span>
          </div>
  ...
  if (item.type === "mentioned_you" || item.type === "issue_commented") {
    return (
      <div className="bg-slate-50 dark:bg-zinc-950 border border-white/5 rounded-2xl p-4 mb-4 cursor-pointer hover:bg-white dark:bg-zinc-900/80 transition-colors" onClick={() => router.push(`/issues/${item.issueId}`)}>
        <div className="flex items-center gap-3 mb-3">
           <div className="relative w-10 h-10 rounded-full overflow-hidden shrink-0">
              {item.actorPhotoURL ? (
                <Image fill src={item.actorPhotoURL} alt="" className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center text-sm font-bold text-slate-500 dark:text-zinc-500">
                  {item.actorDisplayName.charAt(0)}
                </div>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                ...
                <span className="text-slate-500 dark:text-zinc-500 text-sm truncate">{item.type === "mentioned_you" ? "mentioned you in a comment" : "commented on your issue"}</span>
              </div>
              <div className="text-xs font-medium text-zinc-600">{timeAgo}</div>
            </div>
        </div>
        
        <div className="bg-white dark:bg-zinc-900/50 border border-white/5 rounded-xl p-3 mb-3 text-sm text-slate-600 dark:text-zinc-300">
          &quot;{item.commentPreview}...&quot;
        </div>
  ...
  return (
    <div className="bg-slate-50 dark:bg-zinc-950 border border-white/5 rounded-3xl overflow-hidden mb-6 hover:border-white/10 transition-colors cursor-pointer group" onClick={() => router.push(`/issues/${item.issueId}`)}>
      <div className="p-4 flex items-center gap-3 border-b border-white/5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs font-medium text-slate-500 dark:text-zinc-500">Reported an issue • {timeAgo}</div>
        </div>
      </div>
      
      {item.issueThumbnail && (
        <div className="w-full h-64 bg-white dark:bg-zinc-900 relative">
          <Image fill src={item.issueThumbnail} alt="Issue" className="w-full h-full object-cover" />
        </div>
      )}
      ...
      <div className="p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold text-slate-500 dark:text-zinc-500 capitalize px-2.5 py-1 bg-white dark:bg-zinc-900 rounded-full border border-white/5">{item.issueCategory?.replace('_', ' ')}</span>
        </div>
        ...
        <div className="flex items-center gap-6 text-slate-500 dark:text-zinc-500 font-bold text-sm">
