...
export function UserListItem({ user, mutualCount }: UserListItemProps) {
  ...
  return (
    <div 
      className="flex items-center gap-3 p-3 hover:bg-zinc-900/80 rounded-xl transition-colors cursor-pointer group border border-transparent hover:border-white/5"
      onClick={() => router.push(`/profile/${user.username}`)}
    >
      <div className="relative shrink-0 w-11 h-11 rounded-full overflow-hidden bg-zinc-800 border border-white/10">
        {user.photoURL ? (
      ...
      <div className="flex-1 min-w-0">
        ...
        {user.bio && (
          <div className="text-[13px] text-zinc-400 truncate mt-1">{user.bio}</div>
        )}
