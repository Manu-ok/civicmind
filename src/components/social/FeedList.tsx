  ...
  if (loading && items.length === 0) {
    return (
      <div className="flex flex-col gap-6">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="bg-zinc-950 border border-white/5 rounded-3xl p-5 animate-pulse">
            ...
            <div className="w-full h-48 bg-zinc-900 rounded-2xl mb-4" />
  ...
  if (!loading && items.length === 0) {
    return (
      <motion.div 
        ...
        className="text-center py-20 bg-zinc-950 border border-white/5 rounded-3xl"
      >
        <div className="w-16 h-16 rounded-full bg-zinc-900 mx-auto flex items-center justify-center mb-4 border border-white/5">
        ...
        <p className="text-zinc-500 text-sm max-w-[280px] mx-auto">
          Follow community members, join circles, or change your ward to see local activity.
        </p>
      </motion.div>
    );
  }
