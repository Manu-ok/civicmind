...
export function UserCard({ user }: { user: SocialUser }) {
  ...
  return (
    <div 
      ...
      className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-xl overflow-hidden transition-all duration-200 ease-in-out hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-900/10 cursor-pointer group shadow-lg flex flex-col"
    >
      ...
      <div className="p-5 pt-0 relative flex flex-col flex-1">
        <div className="flex justify-between items-end mb-3 -mt-10">
          <div className="relative w-20 h-20 rounded-full overflow-hidden border-4 border-zinc-900 bg-zinc-800 shrink-0">
            {user.photoURL ? (
              <img src={user.photoURL} alt={user.displayName} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full flex items-center justify-center font-bold text-3xl text-zinc-500">
        ...
        <div className="mb-3">
          ...
          <div className="text-sm font-medium text-zinc-500 truncate">@{user.username}</div>
        </div>
        
        <div className="text-sm text-zinc-400 line-clamp-2 min-h-[40px] flex-1 leading-relaxed">
          {user.bio || "Active citizen in the community."}
        </div>
        
        <div className="mt-5 pt-4 border-t border-zinc-800 flex items-center justify-between text-xs font-medium text-zinc-500">
