    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] md:h-screen w-full relative">
      ...
      <div className="absolute top-4 left-4 z-10 flex gap-2">
        <Button 
          ...
          className="bg-zinc-900/90 backdrop-blur shadow-xl border-white/10"
        >
          <Filter className="w-4 h-4" />
        </Button>
        <div className="hidden md:flex bg-zinc-900/90 backdrop-blur rounded-lg shadow-xl border border-white/10 items-center px-4 py-2 gap-4">
          <span className="text-sm font-medium text-zinc-300">Live Issues: <span className="text-white font-bold">{issues.length}</span></span>
        </div>
      </div>

      {/* Floating Filters Panel */}
      {showFilters && (
        <div className="absolute top-16 left-4 z-20 w-64 bg-zinc-900/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/10 p-4">
          <h3 className="font-semibold text-sm text-zinc-400 uppercase tracking-wider mb-3">Categories</h3>
          ...
          <h3 className="font-semibold text-sm text-zinc-400 uppercase tracking-wider mb-3">Severity</h3>
