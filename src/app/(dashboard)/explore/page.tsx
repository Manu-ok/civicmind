  ...
  const SearchDropdown = () => (
    <div className="absolute top-full left-0 right-0 mt-2 bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
      <div className="p-4 border-b border-white/5">
        <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-3 flex items-center gap-2"><Users className="w-4 h-4" /> People</h4>
        {heroes.slice(0,2).map(u => (
          <div key={u.id} className="flex items-center gap-3 mb-3 cursor-pointer hover:bg-zinc-800 p-2 rounded-xl transition-colors" onClick={() => router.push(`/profile/${u.username}`)}>
             <div className="w-8 h-8 rounded-full bg-zinc-800 flex-shrink-0 overflow-hidden">
               {u.photoURL ? <img src={u.photoURL} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-zinc-800" />}
             </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 relative">
        ...
        <div className="flex-1 max-w-3xl">
          ...
          <div className="mb-8 relative z-40">
            ...
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Search className="h-6 w-6 text-zinc-500 group-focus-within:text-blue-500 transition-colors" />
              </div>
              <input
                type="text"
                className="block w-full pl-12 pr-4 py-4 md:py-5 bg-zinc-950 border-2 border-white/5 rounded-2xl text-lg text-white placeholder-zinc-500 focus:outline-none focus:border-blue-500 transition-all shadow-xl"
            ...
            {TABS.map(tab => (
              <button
                ...
                className={`relative flex items-center gap-2 px-5 py-3 rounded-full font-bold text-sm whitespace-nowrap transition-colors ${
                  activeTab === tab.id ? "text-white" : "text-zinc-500 hover:bg-zinc-900"
                }`}
              >
                {activeTab === tab.id && (
                  <motion.div 
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === "circles" && (
                <div className="space-y-8">
                  <div className="bg-gradient-to-br from-blue-500/20 to-fuchsia-500/20 border border-blue-500/30 rounded-3xl p-8 text-center relative overflow-hidden">
                    ...
                    <p className="text-zinc-400 mb-6 max-w-md mx-auto">Connect with neighbors, organize cleanups, and amplify your voice together.</p>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {['City Hall Updates', 'Green Guardians', 'Traffic Watch'].map((name, i) => (
                        <div key={i} className="bg-zinc-950 border border-white/5 rounded-2xl p-5 hover:border-blue-500/30 transition-colors cursor-pointer group">
                          <div className="flex items-center gap-3 mb-3">
                            <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center border border-blue-500/20">
                              <ShieldCheck className="w-5 h-5 text-blue-500" />
                            </div>
                            <div>
                              <div className="font-bold text-white group-hover:text-blue-400 transition-colors flex items-center gap-1">{name} <CheckCircle2 className="w-3.5 h-3.5 text-blue-500" /></div>
                              <div className="text-xs text-zinc-500">Official • {500 + i * 150} members</div>
                            </div>
                          </div>
                          <div className="text-sm text-zinc-400">Join the official discussion and get real-time updates.</div>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {activeTab === "nearby" && (
                <div className="bg-zinc-950 border border-white/5 rounded-3xl p-8 text-center py-20">
                  ...
                  <p className="text-zinc-500 text-sm max-w-xs mx-auto mb-6">Location services are scanning for recent issues and community members within a 5km radius.</p>
        ...
        <div className="hidden xl:block w-[320px] shrink-0 sticky top-24 h-[calc(100vh-6rem)]">
          ...
          <div className="mt-6 bg-gradient-to-b from-zinc-900 to-zinc-950 border border-white/5 rounded-3xl p-5">
            ...
            <p className="text-sm text-zinc-400">Reporting issues with clear photos and exact locations increases resolution speed by 40%.</p>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((item, i) => (
          <div key={i} className="bg-zinc-950 border border-white/5 hover:border-white/10 transition-colors rounded-2xl p-4 cursor-pointer group">
            <div className="flex items-start justify-between mb-3">
              ...
              <span className="text-xs font-bold text-zinc-500 flex items-center gap-1"><Flame className="w-3.5 h-3.5 text-orange-500" /> {item.engagement}</span>
            </div>
