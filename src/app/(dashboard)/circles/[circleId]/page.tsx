    ...
    <div className="min-h-screen bg-background pb-24 md:pb-12">
      ...
      <div className="h-64 md:h-80 w-full bg-gradient-to-br from-blue-500/20 to-fuchsia-500/20 relative">
        ...
        <div className="absolute -bottom-16 left-0 right-0 max-w-5xl mx-auto px-4 md:px-6 flex flex-col md:flex-row items-end md:items-center gap-6">
           <div className="w-32 h-32 md:w-40 md:h-40 rounded-3xl bg-zinc-900 border-4 border-background flex items-center justify-center text-6xl shadow-2xl z-10 shrink-0">
             {circle.iconEmoji}
           </div>
           ...
           <div className="flex-1 pb-2 w-full">
             <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 w-full">
               ...
               <div className="flex items-center gap-3 shrink-0">
                 {isAdmin && (
                   <button className="p-3 bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl transition-colors border border-white/5">
                 ...
                 <button 
                    ...
                    className={`px-8 py-3 rounded-xl font-bold transition-all shadow-xl ${
                      isMember 
                        ? "bg-zinc-900 text-zinc-300 hover:bg-red-500/10 hover:text-red-500 border border-white/5" 
        ...
        <div className="min-h-[400px]">
          <AnimatePresence mode="wait">
            ...
            >
                ...
                <div className="max-w-xl">
                  {isMember ? (
                    <div className="bg-zinc-950 border border-white/5 border-dashed rounded-3xl p-10 text-center">
                      ...
                      <p className="text-zinc-500 mb-6">Issues reported in {circle.ward} will appear here.</p>
                    ...
                    <div className="bg-zinc-950 border border-white/5 border-dashed rounded-3xl p-10 text-center">
                      ...
                      <p className="text-zinc-500">Join this circle to view and participate in discussions.</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {activeTab === "members" && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {membersLoading ? (
                    ...
                    <p className="text-zinc-500 col-span-full">No members yet.</p>
                  ) : (
                    members.map(member => (
                      <div key={member.userId} className="bg-zinc-950 border border-white/5 rounded-2xl p-4 flex items-center gap-3 cursor-pointer hover:bg-zinc-900 transition-colors" onClick={() => router.push(`/profile/${member.username}`)}>
                        <div className="w-12 h-12 rounded-full bg-zinc-800 overflow-hidden shrink-0 border border-white/5">
                          {member.photoURL ? (
                            <img src={member.photoURL} alt="" className="w-full h-full object-cover" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center font-bold text-zinc-500">{member.displayName.charAt(0)}</div>
                          )}
                        </div>
              ...
              {activeTab === "about" && (
                <div className="max-w-2xl bg-zinc-950 border border-white/5 rounded-3xl p-8">
                  <h3 className="text-lg font-black text-white mb-4">About this Circle</h3>
                  <p className="text-zinc-300 leading-relaxed mb-8 whitespace-pre-wrap">{circle.description}</p>
                  
                  <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/5">
                    <div>
                      <div className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1">Created</div>
